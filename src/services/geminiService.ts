...
export const runPipeline = async (params: BuildUIParams): Promise<EngineOutput> => {
  ...
  if (params.step === "images") {
    if (!params.plan_json?.plan_json) {
      return { status: "error", step: "images", error_code: "plan_json_missing", message: "Run Step 1 first." };
    }
    ...
    return { status: "ok", step: "images", images: generatedImages };
  }

  return { status: "error", step: "text_plan", message: "Unknown step." };
};
